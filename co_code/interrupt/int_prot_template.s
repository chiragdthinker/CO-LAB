...
;	mov esp,0xc5e0
	...
	;div by zero; INT VECTOR 0
	mov word [DS:0x4],0xaaaa
	mov word [DS:0x8],0xbbbb
	div EBX
	mov [DS:0x0],EAX

	;breakpoint trap; INT VECTOR 3
	mov word [DS:0xc],0xaaaa
	mov word [DS:0x10],0xbbbb
	int 3;
	mov word [DS:0x10],0xcafe
	
	;Segment not present; INT VECTOR 11
	mov word [DS:0x14],0xaaaa
	mov word [DS:0x18],0xbbbb
	mov eax,0x30
	mov es,eax
	mov word [ES:0x4],0xabcd

	;Writing to a out of bound memory location
	mov word [DS:0x1c],0xaaaa
	mov word [DS:0x20],0xbbbb
	mov eax,0x30
	mov es,eax
	mov word [ES:0x300],0xabcd

	jmp chanHappy

	align 0x200
	mov EBX,0x3;
	mov word [DS:0x4],0xbabe
	mov word [DS:0x8],0xcafe
	iret

	align 0x100
	mov word [DS:0xc],0xbabe
	mov word [DS:0x10],0xcafe
	iret

	align 0x100
	pop eax
	mov word [DS:0x14],0xbabe
	mov word [DS:0x18],0xcafe
	mov eax,0x20
	mov es,eax
	mov byte [es:5], 0x92
	iret
	...
	align 0x100
	pop eax
	mov word [DS:0x1c],0xbabe
	mov word [DS:0x20],0xcafe
	mov eax,0x28
	mov es,eax
	iret
...
;Data Segment Descriptor of type 0, random DPL and Base
 dd 0xb000031f , 0x409200		;using it as normal write access mem
; Offset 0x20
;Descriptor for GDT entry corresponding to the segment descriptor whose present bit is 0 
 dd 0xd0300008 , 0x00409200
; Offset 0x28
;Data Segment Descriptor of type 3, random DPL and Base
 dd 0xc5e0098f , 0x409600	;stack segment discriptor
; Offset 0x30
;Data Segment Descriptor of type 1, random DPL and Base
 dd 0xb3200050 , 0x401200		;present bit set as 0 to generate exception
;------------KAMA - PUT CODE SEGMENT DESCRIPTOR FOR ISR------
; Offset 0x38
; dd 0xbc80095f , 0x409a00		;using it as execute and read type mem
  dd 0xa0001000
  dd 0x00409a00				;read/execute permission for code

;Offset 0x40
;dummy segment descriptor; since LDT is location 0x58, we are adding a segment descriptor here
 dd 0xb32004af , 0x409200		
