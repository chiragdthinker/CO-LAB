...
L1:
	...
	;mov esi, 0x15e0+(edi*0x1E+ebx)*0x8
		...
		;mov esi,(0x14*edi+ecx)*0x4
			...
			imul edx  ;multiplying eax and edx which will be stored in edx:eax
			...
			;getting the last 4 bytes of C[edi][ebx]
			;mov esi,0xC80+(0x1E*edi+ebx)*0x8+0x4
			...
			;adding C[edi][ebx] and the product A[edi][ecx]*B[ecx][ebx]
			
			add eax,esp
			
			;using adc for doing 64 bit addition
			
			adc edx,[esi]

			;storing first 32 bits in edx and last 32 bits in eax 
			...
			add esi,0x15e0
			;mov esi,0xC80+(0x1E*edi+ebx)*0x8+0x4
			mov [esi],eax		;storing the result back in C[edi][ebx] last 32 bits
			...
			mov [esi],edx		;storing the result back in C[edi][ebx] first 32 bits
